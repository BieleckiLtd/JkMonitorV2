    ...
    <div className='rounded-lg border border-border/50 bg-background/40 px-3 py-2'>
      <div className={cn(
        ...
        <span className='flex min-w-0 flex-wrap items-center gap-1.5 text-xs text-muted-foreground'>
          ...
          {statusChip ? (
            <span className={cn('rounded-full border px-2 py-0.5 text-[10px] font-semibold tracking-[0.12em]', statusChip.className)}>
              {statusChip.label}
            </span>
          ) : null}
        </span>
...
function getSwitchStatusChip(
  ...
  switch (param.key) {
    case 'charge_switch': {
      ...
      return buildSwitchStatusChip(isActive, 'Active', 'Not Active');
    }
    case 'discharge_switch': {
