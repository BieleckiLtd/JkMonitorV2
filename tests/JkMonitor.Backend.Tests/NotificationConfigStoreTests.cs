...
{
    private readonly string _tempRootPath = Path.Combine(Path.GetTempPath(), $"jkmonitor-notifications-{Guid.NewGuid():N}");
    ...
    public void LoadLegacyConfigFromFile_ParsesNotificationConfig()
    {
        ...
        Assert.Equal("jkmonitor-alerts", channel.Settings["topic"]?.ToString());
