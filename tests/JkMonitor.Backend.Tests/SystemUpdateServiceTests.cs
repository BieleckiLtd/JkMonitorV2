...
public class SystemUpdateServiceTests
{
    ...
    public async Task CheckForUpdateAsync_UsesInstallerChecksumKey()
    {
        using var releaseInfoScope = TemporaryReleaseInfoScope.Create(
            "JKMONITOR_RELEASE_SHA256=1111111111111111111111111111111111111111111111111111111111111111");
        ...
        var service = CreateService(
            new StubHttpClientFactory(new StubHttpMessageHandler(request =>
            {
                    ...
                    return CreateJsonResponse("""
                        {
                          ...
                          "assets": [
                            {
                              "name": "jkmonitor-backend-linux-arm64.tar.gz.sha256",
                              "browser_download_url": "https://example.test/jkmonitor-backend-linux-arm64.tar.gz.sha256"
                            }
                          ]
                        }
                        """);
                ...
                if (request.RequestUri?.AbsoluteUri == "https://example.test/jkmonitor-backend-linux-arm64.tar.gz.sha256")
                {
                    return new HttpResponseMessage(HttpStatusCode.OK)
                    {
                        Content = new StringContent("2222222222222222222222222222222222222222222222222222222222222222  jkmonitor-backend-linux-arm64.tar.gz")
                    };
                }
        ...
        var service = CreateService(
            new StubHttpClientFactory(new StubHttpMessageHandler(request =>
            {
                ...
                if (request.RequestUri?.AbsoluteUri == "https://example.test/jkmonitor-backend-linux-arm64.tar.gz.sha256")
                {
                    return new HttpResponseMessage(HttpStatusCode.OK)
                    {
                        Content = new StringContent("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb  jkmonitor-backend-linux-arm64.tar.gz")
                    };
                }
